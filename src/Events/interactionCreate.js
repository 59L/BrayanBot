...
/** @param {BryanBot} bot @param {Interaction} interaction */
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (cmd && cmd.commandData.Enabled == true) {
      if (cmd.commandData.Cooldown && !interaction.user.bot) {
        ...
        const isOnCooldown = Cooldown.getCooldown(
            ...
            Object.assign(
              Utils.setupMessage({
                ...
                variables: [
                  ...
                  {
                    searchFor: /{cooldown}/g,
                    replaceWith: ms(isOnCooldown.time - Date.now(), {
                      long: true,
                    }),
                  },
                  {
                    searchFor: /{time}/g,
                    replaceWith: Math.ceil(isOnCooldown.time / 1000),
                  },
                  {
                    searchFor: /{command}/g,
                    replaceWith: command.slice(config.Settings.Prefix.length),
                  },
                ],
              }),
              { ephemeral: true }
      ...
      if (cmd.commandConfig.requiredPermissions.user && cmd.commandConfig.requiredPermissions.user.length >= 1) {
        if (
          !interaction.guild?.members.cache
            ...
            ?.permissions.has(cmd.commandConfig.requiredPermissions.user)
        ) {
          return interaction.reply(
            Utils.setupMessage({
      ...
      if (
        cmd.commandData.Permission &&
        !Utils.hasPermission(cmd.commandData.Permission, interaction.member)
      ) {
        return interaction.reply(
          Utils.setupMessage({
